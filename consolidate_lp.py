...
logger = logging.getLogger("LPConsolidator")
...
def process_zip_dataset(zip_path: Path, ds_prefix: str, target_classes: list, class_mapping: dict, seen_hashes: set):
    ...
    with zipfile.ZipFile(zip_path, 'r') as zh:
        ...
        for img_zip_path_str in image_files:
            ...
            # Only keep images that have at least one valid license plate box!
            if valid_lines:
                try:
                    # 1. Read image bytes in-memory and decode
                    img_bytes = zh.read(img_zip_path_str)
                    
                    # Compute MD5 hash for de-duplication
    ...
    logger.info(f"✓ {ds_prefix} completed: {len(results)} valid LP images, skipped {duplicate_count} duplicates.")
    return results

def main():
    root_dir = Path("c:/Users/Lenovo/Documents/cv_project")
    lp_data_dir = root_dir / "lp_detection_last"
    
    # 1. Dataset configurations with class mappings to index 0
    zips = {
        "lp_ds1": ("Helmet and Number plat.v1-cv.yolov11.zip", [0], {0: 0}),
        "lp_ds2": ("Helmet and Number Plate Detection for Motorbike Safety.v1-cv.yolov11.zip", [3], {3: 0}),
        "lp_ds3": ("Helmet detection.v1-vc.yolov11.zip", [3], {3: 0}),
        "lp_ds4": ("HelmetDetectionDataset.v1-cv.yolov11.zip", [0], {0: 0})
    }
    ...
    for ds_prefix, (zip_name, target_classes, mapping) in zips.items():
        zip_path = lp_data_dir / zip_name
        if not zip_path.exists():
            logger.error(f"Zip file not found: {zip_path}")
            continue
        
        records = process_zip_dataset(zip_path, ds_prefix, target_classes, mapping, seen_hashes)
    ...
    logger.info(f"Total Unique LP Images: {total_imgs}")
    logger.info(f"Total LP Bounding Boxes: {total_boxes}")
    logger.info(f"========================================\n")
    ...
    # Clean any existing MASTER_LP split zips to prevent mismatches
    for zip_file in output_dir.glob("MASTER_LP_PART_*.zip"):
        logger.info(f"Removing old zip file: {zip_file}")
        zip_file.unlink()
        
    for p_idx in range(num_parts):
        part_num = p_idx + 1
        part_zip_path = output_dir / f"MASTER_LP_PART_{part_num}.zip"
        ...
        temp_part_dir = root_dir / f"temp_lp_part_{part_num}"
        ...
        nested_img = temp_part_dir / "MASTER_LP" / "images"
        nested_lbl = temp_part_dir / "MASTER_LP" / "labels"
    ...
    logger.info("\n✓ All 5 master LP parts created successfully under training_lab/hf_cv_space/data/!")
