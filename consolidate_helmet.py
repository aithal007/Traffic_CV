...
logger = logging.getLogger("HelmetConsolidator")
...
def process_zip_dataset(zip_path: Path, ds_prefix: str, target_classes: list, class_mapping: dict):
    ...
    with zipfile.ZipFile(zip_path, 'r') as zh:
        ...
        for img_zip_path_str in image_files:
            ...
            # If we have valid helmet/no-helmet annotations, process image and append!
            if valid_lines:
                try:
    ...
    logger.info(f"✓ {ds_prefix} completed: {len(results)} valid helmet images.")
    return results

def main():
    root_dir = Path("c:/Users/Lenovo/Documents/cv_project")
    helmet_data_dir = root_dir / "helmet_best_last"
    
    # 1. Dataset configurations
    zips = {
        "ds1": ("Motorcycle Helmet Detection.v1-cv.yolov11.zip", [0, 1, 3], {0: 0, 1: 0, 3: 1}), # 0,1 -> 0 (with_helmet), 3 -> 1 (no_helmet)
        "ds2": ("helmet-detection-20241116.v1-vc.yolov11.zip", [2, 3], {2: 0, 3: 1})          # 2 -> 0 (with_helmet), 3 -> 1 (no_helmet)
    }
    
    all_records = []
    
    for ds_prefix, (zip_name, target_classes, mapping) in zips.items():
        zip_path = helmet_data_dir / zip_name
    ...
    logger.info(f"Total Unique Helmet Images: {total_imgs}")
    logger.info(f"Total Helmet Bounding Boxes: {total_boxes}")
    logger.info(f"========================================\n")
    ...
    # Clean any existing MASTER_HELMET split zips to prevent mismatches
    for zip_file in output_dir.glob("MASTER_HELMET_PART_*.zip"):
        logger.info(f"Removing old zip file: {zip_file}")
        zip_file.unlink()
        
    for p_idx in range(num_parts):
        part_num = p_idx + 1
        part_zip_path = output_dir / f"MASTER_HELMET_PART_{part_num}.zip"
        ...
        temp_part_dir = root_dir / f"temp_helmet_part_{part_num}"
        ...
        nested_img = temp_part_dir / "MASTER_HELMET" / "images"
        nested_lbl = temp_part_dir / "MASTER_HELMET" / "labels"
